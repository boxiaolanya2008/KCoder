...
- `kcoder serve` command
- The desktop app itself

**Only** add if it's specifically about the Desktop application or `kcoder serve` view. **Do not** add for terminal, TUI, or general kcoder issues.
...
**Only** add if the issue mentions "zen" or "kcoder zen" or "kcoder black".

If the issue doesn't have "zen" or "kcoder black" in it then don't add zen label

#### core

Use for core server issues in `packages/kcoder/`, excluding `packages/kcoder/src/cli/cmd/tui/`.
...
TUI (`packages/kcoder/src/cli/cmd/tui/...`):
...
Core (`packages/kcoder/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for kcoder server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
