...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@kcoder/acme")).toBe("@kcoder/acme")
    expect(Npm.sanitize("@kcoder/acme@1.0.0")).toBe("@kcoder/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/kcoder/acme.git"
    const expected = win ? "acme@git+https_//github.com/kcoder/acme.git" : spec
