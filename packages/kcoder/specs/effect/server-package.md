...
Practical reference for extracting a future `packages/server` from the current `packages/kcoder` monolith while `packages/core` is still being migrated to Effect.
...
- `packages/core` - all kcoder services, Effect-first source of truth
- `packages/server` - kcoder server, with separate contract and implementation, still producing `openapi.json`
...
- import from `server` and embed the full kcoder server into an existing Node app
- spawn the CLI and talk to the server through that boundary

## Current state

Everything still lives in `packages/kcoder`.
...
- those slices are mounted into the Hono server behind `KCODER_EXPERIMENTAL_HTTPAPI`
...
The intended direction here is the same, but the current `kcoder` package split is earlier in the migration.
...
- the current server host still lives in `packages/kcoder`
...
- if `packages/server` imports services from `packages/kcoder` while `packages/kcoder` imports `packages/server` to host routes, we create a package cycle immediately
...
4. keep `packages/kcoder` as the temporary Hono host
...
- `packages/server` must not import from `packages/kcoder`

Allowed in phase 1:

- `packages/kcoder` imports `packages/server`
...
- `packages/kcoder` shrinks or disappears as package responsibilities are fully split
...
- `packages/kcoder` keeps generating legacy OpenAPI from Hono routes
...
- expose host adapters needed by `packages/kcoder`
...
Keep in `packages/kcoder` for now:
...
- host mounting from `packages/kcoder`
...
- no imports from `packages/kcoder` inside `packages/server`
...
- `packages/server` must still not import from `packages/kcoder`
...
### PR 4. Mount `packages/server` question from `packages/kcoder`

Scope:

- replace local experimental question route wiring in `packages/kcoder`
...
- mount it in parallel from `packages/kcoder`
...
- move adapter selection and server startup out of `packages/kcoder`

Rules:

- do not start this while `packages/server` still depends on `packages/kcoder`
...
- host logic no longer lives in `packages/kcoder`
...
3. the route is mounted from the current host in `packages/kcoder`
...
- `packages/server` imports services or runtime from `packages/kcoder`
- `packages/kcoder` imports route definitions or handlers from `packages/server`

Avoid by:

- keeping phase-1 `packages/server` free of `packages/kcoder` imports
...
- [ ] mount `question` from `packages/kcoder`
...
2. keep `packages/kcoder` as the temporary host
...
If a proposed PR would make `packages/server` import from `packages/kcoder`, stop and restructure the boundary first.
