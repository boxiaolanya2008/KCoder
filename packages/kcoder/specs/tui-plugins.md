...
- Author package entrypoint is `@kcoder/plugin/tui`.
...
```json
{
  "$schema": "https://kcoder.ai/tui.json",
  "theme": "smoke-theme",
  "plugin": ["@acme/kcoder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  }
}
...
- Import types from `@kcoder/plugin/tui`.
- `@kcoder/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@kcoder/plugin/tui"
...
- The value is a semver range checked against the running KCoder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.kcoder` is absent, no check is performed (backward compatible).
...
- `KCoder plugin <module>` and TUI install both run install *?manifest read *?config patch.
- Alias: `KCoder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.kcoder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.kcoder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.kcoder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.kcoder` dir or `KCODER_CONFIG_DIR`, KCoder installs `@kcoder/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@kcoder/plugin/tui`.
...
- Local plugins persist installed themes under the local `.kcoder/themes` area near the plugin config source.
...
- `--pure` / `KCODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.kcoder/plugins/tui-smoke.tsx`
- Local vim plugin: `.kcoder/plugins/tui-vim.tsx`
- Local smoke config: `.kcoder/tui.json`
- Local smoke theme: `.kcoder/plugins/smoke-theme.json`
