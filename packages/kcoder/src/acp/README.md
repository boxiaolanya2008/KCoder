...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for kcoder.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal kcoder sessions
...
kcoder acp --cwd /path/to/project
```
...
```bash
KCODER_ENABLE_QUESTION_TOOL=1 kcoder acp
```
...
- **Terminal Integration**: Full terminal support via kcoder's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | kcoder acp
```
...
ACP sessions map cleanly to kcoder's internal session model:
