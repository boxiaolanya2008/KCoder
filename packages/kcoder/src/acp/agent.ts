...
import type { AssistantMessage, Event, KcoderClient, SessionMessageResponse, ToolPart } from "@kcoder/sdk/v2"
...
export async function init({ sdk: _sdk }: { sdk: KcoderClient }) {
  return {
...
export class Agent implements ACPAgent {
  ...
  async initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    const authMethod: AuthMethod = {
      description: "Run `kcoder auth login` in the terminal",
      name: "Login with kcoder",
      id: "kcoder-login",
    }
  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // KCoder stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const opencodeProvider = providers.find((p) => p.id === "kcoder")
  if (opencodeProvider) {
    if (opencodeProvider.models["big-pickle"]) {
      return { providerID: ProviderID.kcoder, modelID: ModelID.make("big-pickle") }
    }
