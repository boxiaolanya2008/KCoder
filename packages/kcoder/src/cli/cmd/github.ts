...
const AGENT_USERNAME = "kcoder-agent[bot]"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.kcoder.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  opencode:
    ...
    steps:
      ...
      - name: Run opencode
        uses: anomalyco/kcoder/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.kcoder.ai" : "https://opencode.ai"
      ...
      try {
        ...
        console.log("kcoder session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.kcoder.ai"
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `kcoder/${type}-${hex}-${timestamp}`
        }
        return `kcoder/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        ...
        const shareUrl = shareId ? `[kcoder session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
