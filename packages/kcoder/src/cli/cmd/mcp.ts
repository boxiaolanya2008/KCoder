    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: kcoder mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in kcoder.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .kcoder/ subdirectory too)
  const candidates = [path.join(baseDir, "kcoder.json"), path.join(baseDir, "kcoder.json")]

  if (!global) {
    candidates.push(path.join(baseDir, ".kcoder", "kcoder.json"), path.join(baseDir, ".kcoder", "kcoder.json"))
  }
  ...
  // Default to kcoder.json if none exist
  return candidates[0]
}
