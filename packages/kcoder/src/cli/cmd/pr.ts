...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run KCoder",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for KCoder session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found kcoder session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["kcoder", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting kcoder...")
        UI.println()

        const kcoderArgs = sessionId ? ["-s", sessionId] : []
        const kcoderProcess = Process.spawn(["kcoder", ...kcoderArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await kcoderProcess.exited
        if (code !== 0) throw new Error(`kcoder exited with code ${code}`)
      },
    })
  },
})
