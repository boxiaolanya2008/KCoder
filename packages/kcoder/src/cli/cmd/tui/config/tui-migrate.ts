...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from kcoder.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "kcoder"),
    ...(await Filesystem.findUp(["kcoder.json", "kcoder.json"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "kcoder"))
  }
  if (Flag.KCODER_CONFIG) files.push(Flag.KCODER_CONFIG)
