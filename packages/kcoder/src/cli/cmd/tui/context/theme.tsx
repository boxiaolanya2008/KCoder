...
import kcoder from "./theme/kcoder.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@kcoder/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "kcoder")
        draft.active = typeof active === "string" ? active : "kcoder"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.kcoder, store.mode)
    })
