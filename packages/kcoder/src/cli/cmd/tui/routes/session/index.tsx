  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}kcoder -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  return (
    ...
    >
      <box flexDirection="row" width="100%" height="100%">
        <box flexGrow={1} flexDirection="column" height="100%" paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
          <Show when={session()}>
            <scrollbox
              ref={(r) => (scroll = r)}
              viewportOptions={{
...
function Write(props: ToolProps<typeof WriteTool>) {
  ...
  const isRunning = createMemo(() => props.part.state.status === "running")
  const title = createMemo(() => {
    const path = normalizePath(props.input.filePath!)
    if (isRunning()) return "Writing " + path + "..."
    return "# Wrote " + path
  })
