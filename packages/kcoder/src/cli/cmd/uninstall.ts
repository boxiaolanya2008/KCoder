...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall kcoder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall KCoder")
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      npm: ["npm", "uninstall", "-g", "kcoder"],
      pnpm: ["pnpm", "uninstall", "-g", "kcoder"],
      bun: ["bun", "remove", "-g", "kcoder"],
      yarn: ["yarn", "global", "remove", "kcoder"],
      brew: ["brew", "uninstall", "kcoder"],
      choco: ["choco", "uninstall", "kcoder"],
      scoop: ["scoop", "uninstall", "kcoder"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "kcoder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using KCoder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".kcoder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".kcoder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".kcoder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".kcoder"))
    ) {
      continue
    }
