...
import { NamedError } from "@kcoder/core/util/error"
import { Flag } from "@kcoder/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { Instance, type InstanceContext } from "../project/instance"
import { InstallationLocal, InstallationVersion } from "@kcoder/core/installation/version"
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServer.Server).annotate({
    description: "Server configuration for kcoder serve and web commands",
  }),
...
export class Service extends Context.Service<Service, Interface>()("@kcoder/Config") {}

function globalConfigFile() {
  const candidates = ["kcoder.json", "kcoder.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      let result: Info = pipe(
        ...
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "kcoder.json"))),
        mergeDeep(yield* loadFile(path.join(Global.Path.config, "kcoder.json"))),
      )
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "KCODER_CONFIG_CONTENT") return "local"
        ...
        if (Flag.KCODER_CONFIG) {
          yield* merge(Flag.KCODER_CONFIG, yield* loadFile(Flag.KCODER_CONFIG))
          log.debug("loaded custom config", { path: Flag.KCODER_CONFIG })
        }

        if (!Flag.KCODER_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("kcoder", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.KCODER_CONFIG_DIR) {
          log.debug("loading config from KCODER_CONFIG_DIR", { path: Flag.KCODER_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void, never>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".kcoder") || dir === Flag.KCODER_CONFIG_DIR) {
            for (const file of ["kcoder.json", "kcoder.json"]) {
          ...
          // Auto-discovered plugins under `.kcoder/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.KCODER_CONFIG_CONTENT) {
          const source = "KCODER_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.KCODER_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          log.debug("loaded custom config from KCODER_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["KCODER_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("KCODER_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.KCODER_PERMISSION) {
          result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.KCODER_PERMISSION))
        }
        ...
        if (Flag.KCODER_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.KCODER_DISABLE_PRUNE) {
