...
const MANAGED_PLIST_DOMAIN = "ai.kcoder.managed"

// Keys injected by macOS/MDM into the managed plist that are not KCoder config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/kcoder"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "kcoder")
    default:
      return "/etc/kcoder"
  }
}
...
export function managedConfigDir() {
  return process.env.KCODER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
