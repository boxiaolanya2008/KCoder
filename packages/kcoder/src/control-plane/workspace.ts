...
export const create = fn(CreateInput.zod, async (input) => {
  ...
  const env = {
    KCODER_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    KCODER_WORKSPACE_ID: config.id,
    KCODER_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.KCODER_EXPERIMENTAL_WORKSPACES) return
