...
export const DEPRECATED_PLUGIN_PACKAGES = ["KCoder-openai-codex-auth", "KCoder-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, kcoderVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(kcoderVersion) || semver.major(kcoderVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.kcoder
  if (typeof range !== "string") return
  if (!semver.satisfies(kcoderVersion, range)) {
    throw new Error(`Plugin requires kcoder ${range} but running ${kcoderVersion}`)
  }
}
