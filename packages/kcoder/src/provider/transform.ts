...
export const OUTPUT_TOKEN_MAX = Flag.KCODER_EXPERIMENTAL_OUTPUT_TOKEN_MAX || 32_000
...
export function options(input: {
  ...
  // Anthropic's eager_input_streaming field breaks many non-native endpoints
  // (GitHub Copilot shim, proxies, etc). Default to safe behavior.
  if (
  ...
  if (
    ...
    (input.model.providerID === "kcoder" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("kcoder")) {
      result["promptCacheKey"] = input.sessionID
