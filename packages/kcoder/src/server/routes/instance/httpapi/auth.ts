...
export class Authorization extends HttpApiMiddleware.Service<Authorization>()(
  "@kcoder/ExperimentalHttpApiAuthorization",
  {
    error: Unauthorized,
    security: {
...
function validateCredential<A, E, R>(
  ...
  return Effect.gen(function* () {
    if (!Flag.KCODER_SERVER_PASSWORD) return yield* effect

    if (credential.username !== (Flag.KCODER_SERVER_USERNAME ?? "kcoder")) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    if (Redacted.value(credential.password) !== Flag.KCODER_SERVER_PASSWORD) {
      return yield* new Unauthorized({ message: "Unauthorized" })
    }
    return yield* effect
  })
