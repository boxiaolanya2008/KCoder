...
import { memoMap } from "@kcoder/core/effect/memo-map"
...
const Headers = Schema.Struct({
  ...
  "x-kcoder-directory": Schema.optional(Schema.String),
})
  ...
  Effect.gen(function* () {
    return (effect) =>
      Effect.gen(function* () {
        ...
        const raw = query.directory || headers["x-kcoder-directory"] || process.cwd()
