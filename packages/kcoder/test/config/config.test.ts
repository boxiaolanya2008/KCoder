...
import { EffectFlock } from "@kcoder/core/util/effect-flock"

import { Instance } from "../../src/project/instance"
import { Auth } from "../../src/auth"
import { Account } from "../../src/account/account"
import { AccessToken, AccountID, OrgID } from "../../src/account/schema"
import { AppFileSystem } from "@kcoder/core/filesystem"
import { Env } from "../../src/env"
import { provideTmpdirInstance } from "../fixture/fixture"
import { tmpdir } from "../fixture/fixture"
import { CrossSpawnSpawner } from "@kcoder/core/cross-spawn-spawner"
...
const managedConfigDir = process.env.KCODER_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "kcoder.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "kcoder.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://kcoder.ai/config.json",
      snapshot: false,
    })
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "kcoder.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "kcoder.jsonc")
...
test("ignores legacy tui keys in kcoder config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://kcoder.ai/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "kcoder.json"))
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["KCODER_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { kcoder: { options: { apiKey: "{env:KCODER_CONSOLE_TOKEN}" } } },
        }),
      ),
    ...
    await provideTmpdirInstance(() =>
      Config.Service.use((svc) =>
        Effect.gen(function* () {
          ...
          expect(config.provider?.["kcoder"]?.options?.apiKey).toBe("st_test_token")
        }),
      ),
    ).pipe(Effect.scoped, Effect.provide(layer), Effect.provide(Npm.defaultLayer), Effect.runPromise)
  } finally {
    if (originalControlToken !== undefined) {
      process.env["KCODER_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["KCODER_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "kcoder.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .kcoder directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kcoderDir = path.join(dir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })
      const agentDir = path.join(kcoderDir, "agent")
...
test("agent markdown permission config preserves user key order", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const agentDir = path.join(dir, ".kcoder", "agent")
      await fs.mkdir(agentDir, { recursive: true })
...
test("loads agents from .kcoder/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kcoderDir = path.join(dir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })

      const agentsDir = path.join(kcoderDir, "agents")
...
test("loads commands from .kcoder/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kcoderDir = path.join(dir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })

      const commandDir = path.join(kcoderDir, "command")
...
test("loads commands from .kcoder/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const kcoderDir = path.join(dir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })

      const commandsDir = path.join(kcoderDir, "commands")
...
test("does not try to install dependencies in read-only KCODER_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.KCODER_CONFIG_DIR
  process.env.KCODER_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.KCODER_CONFIG_DIR
    else process.env.KCODER_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable KCODER_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "kcoder.json"),
        JSON.stringify({ $schema: "https://kcoder.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .kcoder config
      const projectDir = path.join(dir, "project")
      const kcoderDir = path.join(projectDir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })
      ...
      // Local .kcoder config with different plugins
      await Filesystem.write(
        path.join(kcoderDir, "kcoder.json"),
        JSON.stringify({
          $schema: "https://kcoder.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kcoder config with some overlapping plugins
      await Filesystem.write(
        path.join(kcoderDir, "kcoder.json"),
        JSON.stringify({
          $schema: "https://kcoder.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".kcoder")
...
// Note: preload.ts sets KCODER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .kcoder config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .kcoder directory config enables it
      const kcoderDir = path.join(dir, ".kcoder")
      await fs.mkdir(kcoderDir, { recursive: true })
      await Filesystem.write(
        path.join(kcoderDir, "kcoder.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  globalThis.fetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/kcoder")) {
      fetchedUrl = urlStr
    ...
    await provideTmpdirInstance(
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            const config = yield* svc.get()
            expect(fetchedUrl).toBe("https://example.com/.well-known/kcoder")
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "kcoder.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-kcoder@2.4.3", file)).toBe("oh-my-kcoder@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-kcoder@2.4.3", "file:///project/.kcoder/plugin/oh-my-kcoder.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.kcoder/plugin/demo.ts", "file:///project/.kcoder/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.kcoder/plugin/demo.ts"])
  })
  ...
  test("loads auto-discovered local plugins as file urls", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const kcoderDir = path.join(projectDir, ".kcoder")
        const pluginDir = path.join(kcoderDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("KCODER_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["KCODER_DISABLE_PROJECT_CONFIG"]
    process.env["KCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["KCODER_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["KCODER_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .kcoder/ directories when flag is set", async () => {
    const originalEnv = process.env["KCODER_DISABLE_PROJECT_CONFIG"]
    process.env["KCODER_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .kcoder directory with a command
          const kcoderDir = path.join(dir, ".kcoder", "command")
          await fs.mkdir(kcoderDir, { recursive: true })
          await Filesystem.write(path.join(kcoderDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .kcoder should NOT be in directories list
          const hasProjectKcoder = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectKcoder).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["KCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KCODER_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["KCODER_CONFIG_DIR"]
      process.env["KCODER_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["KCODER_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["KCODER_CONFIG_DIR"]
      } else {
        process.env["KCODER_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("KCODER_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["KCODER_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["KCODER_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["KCODER_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["KCODER_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from KCODER_CONFIG_DIR, not project
...
describe("KCODER_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in KCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KCODER_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["KCODER_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://kcoder.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["KCODER_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["KCODER_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in KCODER_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["KCODER_CONFIG_CONTENT"]
  ...
  const config = ConfigParse.effectSchema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "KCoder Managed",
          PayloadIdentifier: "ai.kcoder.managed.test",
          PayloadType: "ai.kcoder.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.effectSchema(
    Config.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://kcoder.ai/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://kcoder.ai/config.json")
})
