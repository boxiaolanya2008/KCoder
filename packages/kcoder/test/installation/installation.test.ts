...
describe("installation", () => {
  describe("latest", () => {
    ...
    test("reads npm versions via registry", async () => {
      ...
      expect(calls).toContain(`https://registry.npmjs.org/kcoder-ai/${InstallationChannel}`)
    })
    ...
    test("reads brew formulae API versions", async () => {
      const layer = testLayer(
        ...
        (cmd, args) => {
          ...
          if (cmd === "brew" && args.includes("--formula") && args.includes("yourname/tap/kcoder")) return ""
          if (cmd === "brew" && args.includes("--formula") && args.includes("kcoder")) return "kcoder"
          return ""
        },
      )
    ...
    test("reads brew tap info JSON via CLI", async () => {
      ...
      const layer = testLayer(
        ...
        (cmd, args) => {
          if (cmd === "brew" && args.includes("yourname/tap/kcoder") && args.includes("--formula")) return "kcoder"
