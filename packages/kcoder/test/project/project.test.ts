...
describe("Project.fromDirectory", () => {
  test("should handle git repository with no commits", async () => {
    ...
    const kcoderFile = path.join(tmp.path, ".git", "kcoder")
    expect(await Bun.file(kcoderFile).exists()).toBe(false)
  })
  ...
  test("should handle git repository with commits", async () => {
    ...
    const kcoderFile = path.join(tmp.path, ".git", "kcoder")
    expect(await Bun.file(kcoderFile).exists()).toBe(true)
  })
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  test("worktree should share project ID with main repo", async () => {
    ...
    try {
      ...
      const cache = path.join(tmp.path, ".git", "kcoder")
...
describe("Project.fromDirectory with bare repos", () => {
  test("worktree from bare repo should cache in bare repo, not parent", async () => {
    ...
    try {
      ...
      const correctCache = path.join(barePath, "kcoder")
      const wrongCache = path.join(parentDir, ".git", "kcoder")
  ...
  test("different bare repos under same parent should not share project ID", async () => {
    ...
    try {
      ...
      const cacheA = path.join(bareA, "kcoder")
      const cacheB = path.join(bareB, "kcoder")
      const wrongCache = path.join(parentDir, ".git", "kcoder")
