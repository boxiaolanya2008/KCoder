...
describe("paramToAttributeKey", () => {
  ...
  test("namespaces non-ID params under kcoder.", () => {
    expect(paramToAttributeKey("name")).toBe("kcoder.name")
    expect(paramToAttributeKey("slug")).toBe("kcoder.slug")
  })
})
...
describe("requestAttributes", () => {
  ...
  test("emits OTel-style <domain>.id for ID-shaped route params", () => {
    ...
    expect(attrs["kcoder.sessionID"]).toBeUndefined()
    expect(attrs["kcoder.messageID"]).toBeUndefined()
    expect(attrs["kcoder.partID"]).toBeUndefined()
  })
  ...
  test("namespaces non-ID params under kcoder. (e.g. mcp :name)", () => {
    const attrs = requestAttributes(
      fakeContext("POST", "http://localhost/mcp/exa/connect", {
        name: "exa",
      }),
    )
    expect(attrs["kcoder.name"]).toBe("exa")
