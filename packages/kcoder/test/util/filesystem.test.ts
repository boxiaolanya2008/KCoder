...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "kcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "kcoder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kcoder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "kcoder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["kcoder.json", "kcoder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "kcoder.json"),
        path.join(tmp.path, "kcoder.jsonc"),
        path.join(project, "kcoder.json"),
        path.join(project, "kcoder.jsonc"),
      ])
    })
  })
