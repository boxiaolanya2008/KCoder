...
import { createKcoder } from "@kcoder/sdk"
...
`)
  ...
  const kcoder = await createKcoder({ port: 0 })
  ...
  try {
    ...
    const session = await kcoder.client.session.create()
    const result = await kcoder.client.session
      .prompt({
